...
usage()
{
  ...
  echo "Example: ./01_transfer.sh --sequence sarscov_geco_run42069"
  printf "\n"
  echo "********************************************************"
  exit 2
}
...
then
  ...
  sshpass -p $gridPASS ssh -T $grid_userIP <<EOF
    rsync -aPvz --info=progress2 -e 'sshpass -p "$storagePASS" ssh -p 22' \
    /data/"$SEQ" \
    "$storage_userIP":/storage/ONT_Runs/drag_and_drop/ 2> /data/err_grid2stor.txt

EOF
  # Copies the stderr to local machine for checking
  sshpass -p $gridPASS ssh -T $grid_userIP "cat /data/err_grid2stor.txt" > err_grid2stor.txt
...
EOF
    ...
    sshpass -p $hpcPASS ssh -p 2222 -T $hpc_userIP "cat /data/geco_proj_dir/raw/RITM/err_stor2hpc1.txt" > err_stor2hpc1.txt
    ...
    then    # Warns if there is an identical folder in the HPC1 then replaces it with a new one.
      ...
      echo "********************************************************"
      printf "\n\n"
      sshpass -p $hpcPASS ssh -p 2222 -T $hpc_userIP "rm /data/geco_proj_dir/raw/RITM/$SEQ"
      sshpass -p $hpcPASS ssh -p 2222 -T $hpc_userIP <<EOF
      ln -s /data/nfs/storage/ONT_Runs/drag_and_drop/$SEQ \
